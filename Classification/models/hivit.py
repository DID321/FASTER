...
class HiViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=nn.LayerNorm, ape=True, rpe=True, patch_norm=True, use_checkpoint=False,hifeat=False,
                 **kwargs):
        super().__init__()
        ...
        self.norm = norm_layer(self.num_features) if self.hifeat else None
...
def hivit_base(num_classes, **kwargs):
    model = HiViT(
        embed_dim=512, depths=[2, 2, 20], num_heads=8, stem_mlp_ratio=3., mlp_ratio=4., num_classes=num_classes, 
...
def HiViT_SupCL(num_classes):
    model = HiViT(
        img_size=224, embed_dim=512, depths=[2, 2, 20], num_heads=8, stem_mlp_ratio=3., in_chans=3, mlp_ratio=4.,
        num_classes=num_classes,hifeat=True,
        ape=True, rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6))

    checkpoint = torch.load(
        # 'J:/CSAR_ATR_Bench_20260223/Classification/weights/saratrx-checkpoint-800.pth',
        'J:/CSAR_ATR_Bench_20260223/Classification/weights/HiViT_SupCL_Prtrain_SOC_new_epoch100.pth',
        map_location='cpu')
