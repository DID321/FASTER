...
from models.resnet import ResNet_34, ResNet_18
from models.vgg import VGGNet
from models.hivit import hivit_base, HiViT_SupCL
...
def parameter_setting():
    ...
    # log 文件夹
    parser.add_argument('--log-path', type=str, default='./logs_100/ConvNeXt_pretrain_EOC_angle')
    # 保存权重文件夹
    parser.add_argument('--save-path', type=str, default='./logs_100/ConvNeXt_pretrain_EOC_angle')
    # 训练图像尺寸
    parser.add_argument('--img-size', type=list, default=[224, 224])
    # 数据集配置文件所在根目录 ./cfg/datasets/EOC_polar.yaml
    parser.add_argument('--data-cfg', type=str, default='./cfg/datasets/EOC_angle.yaml')
    # 多少epoch保存一次权重
    ...
    parser.add_argument('--dist-url', default='file:///J:/CSAR_ATR_Bench_20260223/ConvNeXt_EOC_angle_torch_dist.txt', help='url used to set up distributed training')
...
def main(rank, *args):
    ...
    elif args.model_name == 'HiViT_SupCL':
        model = HiViT_SupCL(num_classes=len(cfg["names"].keys()))
    ...
    optimizer = {
        'Adam': torch.optim.Adam(param_groups, lr=init_lr_fit, betas=(args.momentum, 0.95),
                                 weight_decay=args.weight_decay),
        'AdamW': torch.optim.AdamW(param_groups, lr=init_lr_fit, betas=(args.momentum, 0.95),
                                   weight_decay=args.weight_decay),
