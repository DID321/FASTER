...
class SupCLVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    """ Vision Transformer with support for global average pooling
    """
    def __init__(self, stem_layer=PatchEmbed, stem_configs=dict(), global_pool=False, projection_dim=128, **kwargs):
        super(SupCLVisionTransformer, self).__init__(**kwargs)
...
def supcl_vit_base_patch16(**kwargs):
    model = SupCLVisionTransformer(
        patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), projection_dim=128, **kwargs)
    del model.head
    return model