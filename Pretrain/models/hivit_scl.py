...
class SupCLHiViT(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000,
                 ...
                 norm_layer=nn.LayerNorm, ape=True, rpe=True, patch_norm=True, use_checkpoint=False, hifeat=False,projection_dim=128,
                 **kwargs):
        super().__init__()
...
def supcl_hivit_base(**kwargs):
    model = SupCLHiViT(
        embed_dim=512, depths=[2, 2, 20], num_heads=8, stem_mlp_ratio=3., mlp_ratio=4., hifeat=True,
        rpe=False, norm_layer=partial(nn.LayerNorm, eps=1e-6), projection_dim=128, **kwargs)
    return model
