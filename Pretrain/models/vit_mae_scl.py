...
# from stem import PatchEmbed, ConvPatchEmbed, MlpPatchEmbed
...
class MaskedAutoencoderViTSupCL(nn.Module):
    ...
    def forward_feature(self, x, global_pool=False):
        B = x.shape[0]
        # embed patches
        x = self.patch_embed(x)
    ...
    def forward_supcl(self, x):
        ...
        x = self.forward_feature(x, global_pool=True)
        x = self.supcl_head(x)
...
def supcl_mae_vit_base_patch16_dec512d8b(**kwargs):
    model = MaskedAutoencoderViTSupCL(
        patch_size=16, embed_dim=768, depth=12, num_heads=12,
        decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16,
        mlp_ratio=4, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return model
