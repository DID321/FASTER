...
from Pretrain.models.masked_autoencoder import MaskedAutoencoder
from Pretrain.models.hivit import HiViT, PatchEmbed, PatchMerge, BlockWithRPE
...
class HiViTMaskedAutoencoderSupCL(MaskedAutoencoder, HiViT):
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
                 ...
                 decoder_embed_dim=512, decoder_depth=8, decoder_num_heads=16, hifeat=False, projection_dim=128,
                 **kwargs):
        ...
        #-----------------------------Sup contrastive learning head-----------------------------#
        self.supcl_head = nn.Sequential(
    ...
    def forward_supcl(self, x):
        if self.hifeat:
            x = self.forward_features(x, return_hifeat=True)
            h, m, l = x
            B, N, _ = l.shape
            x = torch.cat([h.reshape(B, N, -1), m.reshape(B, N, -1), l], dim=-1)
            x = self.norm(x)
    ...
    def forward(self, imgs, mask_ratio=0.75, is_supcl=False):
        if is_supcl:
            if imgs.shape[1] == 1:
                return self.forward_supcl(torch.cat([imgs, imgs, imgs], dim=1))
            else:
                return self.forward_supcl(imgs)
        else:
            if imgs.shape[1] == 1:
...
def supcl_mae_hivit_base_dec512d6b(**kwargs):
    model = HiViTMaskedAutoencoderSupCL(
        embed_dim=512, depths=[2, 2, 20], num_heads=8, stem_mlp_ratio=3., mlp_ratio=4., 
