...
def train_one_epoch(model: torch.nn.Module,
    ...
    total_supcl_loss = torch.tensor(0.0, device=device)
    ...
    for data_iter_step, data in enumerate(data_loader_par):
        ...
        mae_loss, pred, mask = model(samples, mask_ratio=args.mask_ratio)

        # sup contrastive learning forward
        embed = model(supcl_images, is_supcl=True)
        ...
        supcl_loss = supcl_cls_loss + supcl_angle_loss + supcl_pitch_loss + supcl_polar_loss + supcl_scene_loss

        loss = args.mae_loss_weight * mae_loss + args.supcl_loss_weight * supcl_loss

        img = samples.clone()

        total_loss += loss
        total_mae_loss += mae_loss
        total_supcl_loss += supcl_loss
        ...
        mae_loss_value_reduce = misc.all_reduce_mean(mae_loss.item())
        supcl_loss_value_reduce = misc.all_reduce_mean(supcl_loss.item())
        ...
        loss_dict = {
            ...
            "mae_loss": mae_loss_value_reduce,
            "supcl_loss": supcl_loss_value_reduce,
        ...
        if misc.is_main_process():
            ...
            # )
            data_loader_par.desc = "[train epoch {}] loss: {:.4f}, mae_loss: {:.4f}, supcl_loss: {:.4f}, supcl_cls_loss: {:.4f}, supcl_angle_loss: {:.4f}, supcl_pitch_loss: {:.4f}, supcl_polar_loss: {:.4f}, supcl_scene_loss: {:.4f}, lr: {:.6f}".format(
                epoch, loss_value_reduce, mae_loss_value_reduce, supcl_loss_value_reduce, supcl_cls_loss_reduce,
                supcl_angle_loss_reduce, supcl_pitch_loss_reduce, supcl_polar_loss_reduce, supcl_scene_loss_reduce, lr
            )
            if (data_iter_step + 1) % print_freq == 0 and log_writer is not None:
                log_writer.append_train_value(epoch, data_iter_step + 1, batch_num, loss_dict, lr)
    ...
    total_mae_loss = misc.all_reduce(total_mae_loss)
    total_supcl_loss = misc.all_reduce(total_supcl_loss)
    ...
    avg_mae_loss = total_mae_loss / batch_num
    avg_supcl_loss = total_supcl_loss / batch_num
    ...
    if misc.is_main_process() and log_writer is not None:
        ...
        log_writer.add_scalar('train_mae_loss', avg_mae_loss, epoch)
        log_writer.add_scalar('train_supcl_loss', avg_supcl_loss, epoch)
