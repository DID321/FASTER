...
class SARDataSet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        polar_str = list(self.sar_polar_dict.keys())[polar_idx]
        # 从lables中找到与当前标签的类别相同的索引/与当前标签的类别不同的索引
        same_cls_idxs = np.where(np.array(self.sar_info_dict["cls"]) == cls_idx)[0]
        diff_cls_idxs = np.where(np.array(self.sar_info_dict["cls"]) != cls_idx)[0]
        # 随机选择一个索引作为负样本
        neg_cls_idx = random.choice(diff_cls_idxs)
        im_neg_cls, _, _ = self.load_image(neg_cls_idx)

        # 从类别相同索引中再次筛选与当前标签的俯仰角相同的索引/与当前标签的俯仰角不同的索引
        same_pitch_idxs = same_cls_idxs[np.where(np.array(self.sar_info_dict["pitch"])[same_cls_idxs] == pitch)[0]]
        diff_pitch_idxs = same_cls_idxs[np.where(np.array(self.sar_info_dict["pitch"])[same_cls_idxs] != pitch)[0]]
        # 从俯仰角相同索引中再次筛选与当前标签的场景相同的索引/与当前标签的场景不同的索引
        same_scene_idxs = same_pitch_idxs[np.where(np.array(self.sar_info_dict["scene"])[same_pitch_idxs] == scene_idx)[0]]
        diff_scene_idxs = same_pitch_idxs[np.where(np.array(self.sar_info_dict["scene"])[same_pitch_idxs] != scene_idx)[0]]
        if diff_scene_idxs.size > 0:
            # 从场景不同索引中随机选择一个与当前索引方位角只差小于10度的索引作为正样本
            same_polar_diff_scene_idxs = diff_scene_idxs[np.where(np.array(self.sar_info_dict["polar"])[diff_scene_idxs] == polar_idx)[0]]
            pos_scene_idxs = same_polar_diff_scene_idxs[np.where(np.abs(np.array(self.sar_info_dict["angle"])[same_polar_diff_scene_idxs] - angle) <= 10)[0]]
            pos_scene_idx = random.choice(pos_scene_idxs) if pos_scene_idxs.size > 0 else random.choice(same_polar_diff_scene_idxs)
        else:
            pos_scene_idx = random.choice(diff_pitch_idxs)
        im_pos_scene, _, _ = self.load_image(pos_scene_idx)

        # 从不同俯仰角中随机选择一个与当前索引方位角只差小于10度的索引作为正样本
        same_polar_diff_pitch_idxs = diff_pitch_idxs[np.where(np.array(self.sar_info_dict["polar"])[diff_pitch_idxs] == polar_idx)[0]]
        pos_pitch_idxs = same_polar_diff_pitch_idxs[np.where(np.abs(np.array(self.sar_info_dict["angle"])[same_polar_diff_pitch_idxs] - angle) <= 10)[0]]
        pos_pitch_idx = random.choice(pos_pitch_idxs) if pos_pitch_idxs.size > 0 else random.choice(same_polar_diff_pitch_idxs)
        im_pos_pitch, _, _ = self.load_image(pos_pitch_idx)

        # 从场景相同索引中再次筛选与当前标签的极化方式相同的索引
        same_polar_idxs = same_scene_idxs[np.where(np.array(self.sar_info_dict["polar"])[same_scene_idxs] == polar_idx)[0]].tolist()
        # 移除当前标签的索引
        same_polar_idxs.remove(index)
        # 从剩余方位角中随机选择一个索引作为正样本
        pos_angle_idx = random.choice(same_polar_idxs)
        im_pos_angle, _, _ = self.load_image(pos_angle_idx)

        # 从相同场景索引中再次筛选与当前标签原始索引相同的索引
        same_raw_idx_files = [Path(p).stem.split("_")[3] for p in np.array(self.im_files)[same_scene_idxs]]
        same_img_raw_idxs = same_scene_idxs[np.where(np.array(same_raw_idx_files) == img_raw_idx)[0]].tolist()
        # same_img_raw_idxs包含四个极化方式的索引, 移除当前标签的索引
        same_img_raw_idxs.remove(index)
        # 从剩余极化方式索引中随机选择一个索引作为正样本
        pos_polar_idx = random.choice(same_img_raw_idxs)
    ...
    def collate_fn(batch):
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "img_neg_cls", "img_pos_scene", "img_pos_pitch", "img_pos_angle", "img_pos_polar", "text_feats"}:
                value = torch.stack(value, 0)
            elif k == "visuals":
                value = torch.nn.utils.rnn.pad_sequence(value, batch_first=True)
            if k in {"band", "cls", "scene", "pitch", "angle", "polar", "neg_cls", "pos_scene", "pos_pitch", "pos_angle", "pos_polar"}:
