...
from Pretrain.models.hivit_scl import supcl_hivit_base
from Pretrain.models.vit_scl import supcl_vit_base_patch16
...
from Pretrain.utils.engine_pretrain_scl import train_one_epoch
from Pretrain.data.datasets import SupCLSARDataSet
from Pretrain.data.augment import RandomResizedCenterCrop
from Pretrain.models.resnet_scl import SupCL_ResNet34
...
def parameter_setting():
    ...
    # 训练batch_size
    parser.add_argument('--batch-size', type=int, default=16, help='Batch size per GPU (effective batch size is batch_size * accum_iter * # gpus')
    # 优化器
    ...
    # log 文件夹
    parser.add_argument('--log-path', type=str, default='./logs/ViT_SupCL_Prtrain_SOC')
    # 保存权重文件夹
    parser.add_argument('--save-path', type=str, default='./logs/ViT_SupCL_Prtrain_SOC')
    # 训练图像尺寸
    ...
    parser.add_argument('--dist-url', default='file:///J:/CSAR_ATR_Bench_20260223/SupCL_Pretrain_torch_dist_init.txt', help='url used to set up distributed training')
...
def main(rank, *args):
    ...
    # model = supcl_hivit_base(norm_pix_loss=args.norm_pix_loss)
    # model = SupCL_ResNet34(feature_dim=128)
    model = supcl_vit_base_patch16()
